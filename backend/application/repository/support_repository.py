...
class SupportRepository:
    ...
    @handle_db_exceptions
    def get_orders_by_tech(self, start_date, end_date):
        ...
        orders_by_tech = (
            g.db_session.query(
                Users.id,
                Users.name,
                func.count(distinct(ServiceOrders.id)).label("total_orders")
            )
            .join(ServiceOrders, ServiceOrders.technician_id == Users.id)
            .join(
                ServiceOrderStatus,
                ServiceOrderStatus.service_order_id == ServiceOrders.id
            )
            .filter(~Users.id.in_([21]))
            .filter(Users.level_id != 1)
            .filter(ServiceOrderStatus.status_id == 8)
            .filter(ServiceOrderStatus.register_at >= start)
            .filter(ServiceOrderStatus.register_at < end_exclusive)

        .group_by(Users.id, Users.name)
            .order_by(func.count(distinct(ServiceOrders.id)).desc())
            .all()
        )
