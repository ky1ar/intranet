...
dev-save:
	...
	if [ -n "$$last_log" ]; then \
		...
		sudo docker logs --since "$$iso_timestamp" api_dev > logs_dev/logs_$$(date +%Y%m%d_%H%M%S).txt 2>&1; \
	else \
		echo "No previous logs found. Saving full logs..."; \
		sudo docker logs api_dev > logs_dev/logs_$$(date +%Y%m%d_%H%M%S).txt 2>&1; \
	fi
